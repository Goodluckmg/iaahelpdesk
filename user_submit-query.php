...
<div class="app-container">
    ...
    <main class="main-content">
        <div class="top-bar"><h1 class="page-title">Submit New Query</h1><div class="date-badge"><i class="far fa-calendar-alt"></i> <span id="currentDate"></span></div></div>
        <div class="widget-card">
            <div class="flex-between"><strong>📝 Submit a new query – IAA Helpdesk</strong></div>
            <form id="queryForm">
                <div class="form-group"><label>Query Title *</label><input type="text" id="qTitle" placeholder="e.g., Missing examination CSC 101" required></div>
                <div class="form-group"><label>Category</label><select id="qCategory"><option>Examination issues</option><option>Fee-related query</option><option>Portal login problem</option><option>Course registration error</option><option>Academic documents</option><option>Other</option></select></div>
                <div class="form-group"><label>Department</label><select id="qDept"><option>Examination & Records</option><option>Finance Office</option><option>ICT Support</option><option>Academic Registry</option><option>Dean of Students</option></select></div>
                <div class="form-group"><label>Priority</label><select id="qPriority"><option>Low</option><option>Medium</option><option>High</option><option>Urgent</option></select></div>
                <div class="form-group"><label>Description *</label><textarea rows="5" id="qDesc" placeholder="Provide full details..."></textarea></div>
                <div style="display:flex; gap:12px; justify-content:end;"><a href="user_index.php" class="btn-primary" style="background:#7f8c8d;">Cancel</a><button type="submit" class="btn-primary">Submit Query</button></div>
            </form>
        </div>
    </main>
</div>
...
<script>
    ...
    document.getElementById('queryForm').addEventListener('submit', (e) => {
        ...
        if (!title || !description) { showMessage('Please fill title and description'); return; }
