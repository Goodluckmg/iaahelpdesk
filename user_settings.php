...
<div class="app-container">
    <aside class="sidebar">
        ...
        <div class="nav-menu">
            ...
            <a href="user_settings.php" class="nav-item active"><i class="fas fa-cog"></i><span class="nav-label">Settings</span></a>
    ...
    <main class="main-content">
        <div class="top-bar"><h1 class="page-title">Settings & Preferences</h1><div class="date-badge"><i class="far fa-calendar-alt"></i> <span id="currentDate"></span></div></div>
        <div class="widget-card"><div class="flex-between"><strong>⚙️ Account Settings</strong></div><div class="form-group"><label>Full Name</label><input type="text" id="settingsName" value="Goodluck"></div><div class="form-group"><label>Email Address (for notifications)</label><input type="email" id="settingsEmail" value="goodluck@iaa.ac.tz"></div><div class="form-group"><label>Phone Number (SMS notifications)</label><input type="text" id="settingsPhone" value="+255 712 345 678"></div><div class="form-group"><label>Default Department for queries</label><select id="defaultDept"><option>ICT Support</option><option>Examination & Records</option><option>Finance Office</option><option>Academic Registry</option></select></div><div class="form-group"><label>Notification Preference</label><select id="notifPref"><option>Email & SMS</option><option>Email only</option><option>SMS only</option><option>None</option></select></div><button id="saveSettingsBtn" class="btn-primary">Save Changes</button></div>
        <div class="widget-card"><div class="flex-between"><strong>🔒 Privacy & Data</strong></div><p>Your data is stored locally on your device. You can clear your data at any time.</p><button id="clearDataBtn" class="btn-primary" style="background:#c0392b; margin-top:10px;">Clear All My Data</button></div>
    </main>
</div>
...
<script>
    ...
    document.getElementById('settingsName').value = loggedUser.name || appData.currentUser.name;
    document.getElementById('settingsEmail').value = loggedUser.email || appData.currentUser.email;
    document.getElementById('settingsPhone').value = loggedUser.phone || appData.currentUser.phone;
    ...
    document.getElementById('clearDataBtn').addEventListener('click', () => { if(confirm('Are you sure? This will delete ALL your tickets and data!')) { localStorage.clear(); showMessage('All data cleared! Page will reload.'); window.location.href = 'user_index.php'; } });
