...
<div class="app-container">
    <aside class="sidebar">
        ...
        <div class="nav-menu">
            ...
            <a href="user_feedback.php" class="nav-item active"><i class="fas fa-star"></i><span class="nav-label">Feedback</span></a>
    ...
    <main class="main-content">
        <div class="top-bar"><h1 class="page-title">Feedback & Rating System</h1><div class="date-badge"><i class="far fa-calendar-alt"></i> <span id="currentDate"></span></div></div>
        <div class="widget-card">
            ...
            <div class="form-group"><label>General feedback / suggestions</label><textarea id="generalFeedback" rows="3" placeholder="Share your experience..."></textarea></div>
...
<script>
    ...
    function renderFeedback() {
        ...
        if (resolved.length === 0) { container.innerHTML = '<p>No resolved tickets yet. After your queries are resolved, you can rate them here.</p>'; return; }
        container.innerHTML = resolved.map(t => `<div class="ticket-item"><strong>#${t.id} - ${escapeHtml(t.title)}</strong><br><small>Department: ${t.department} | Resolved: ${t.date}</small>${t.rating ? `<div style="margin-top: 8px;"><i class="fas fa-star" style="color:#f5b042;"></i> Your rating: ${t.rating}/5</div>` : `<div class="fb-rating" data-id="${t.id}" style="margin-top: 10px;">Rate this resolution: <span class="fb-star" data-rate="1" style="cursor:pointer;">★</span><span class="fb-star" data-rate="2" style="cursor:pointer;">★</span><span class="fb-star" data-rate="3" style="cursor:pointer;">★</span><span class="fb-star" data-rate="4" style="cursor:pointer;">★</span><span class="fb-star" data-rate="5" style="cursor:pointer;">★</span></div>`}</div>`).join('');
        document.querySelectorAll('.fb-rating').forEach(container => { const ticketId = parseInt(container.dataset.id); container.querySelectorAll('.fb-star').forEach(star => star.addEventListener('click', () => { addRating(ticketId, parseInt(star.dataset.rate)); showMessage('⭐ Rating saved! Thank you for your feedback.'); renderFeedback(); })); });
    }
    ...
    document.getElementById('submitFeedbackBtn').addEventListener('click', () => { const feedback = document.getElementById('generalFeedback').value; if (feedback) { showMessage('Thank you for your valuable feedback!'); document.getElementById('generalFeedback').value = ''; } else { showMessage('Please write your feedback before submitting.'); } });
